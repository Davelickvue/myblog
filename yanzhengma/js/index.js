...
function init(){
    ...
    btn.onmousedown = function(e){
        ...
        document.onmouseup = function(){
            ...
            // 允许有 5px 的误差
            if(diffLeft < 10 && diffLeft > -10){
                btn.style.transition = 'all .5s';
                imgBlock.style.transition = 'all .5s';
                // 验证成功
                imgBlock.style.left = imgGap.offsetLeft;
                imgBlock.style.opacity = 0;
                imgGap.style.opacity = 0;

                title.innerHTML = '验证成功';
                title.style.color = '#2ed573';
                document.onmousemove = $('.changeImg').onclick = btn.onmousedown = document.onmouseup = null; // 验证成功后，删除事件
            } else {
                // 验证失败
                slider.style.backgroundColor = '#57606f';
